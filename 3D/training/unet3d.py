...
class UNet3D(nn.Module):
    def __init__(self, scales, kernel_sizes, input_res,
        ...
        self.ConvsDown.append(_ConvBlock3D(in_fmaps, 'in', self.kernel_sizes[0], padding_mode=padding_mode))

        # Intermediate down layers (with MaxPool at the beginning)
        for idown, down_fmaps in enumerate(down_blocks):
            self.ConvsDown.append(_ConvBlock3D(down_fmaps, 'down', self.kernel_sizes[idown + 1],
                padding_mode=padding_mode))

        # Bottom layer (MaxPool at the beginning and Upsample/Deconv at the end)
        self.ConvBottom = _ConvBlock3D(bottom_fmaps, 'bottom', self.kernel_sizes[-1],
        ...
        for iup, up_fmaps in enumerate(up_blocks):
            self.ConvsUp.append(_ConvBlock3D(up_fmaps, 'up', self.kernel_sizes[-2 - iup], 
                padding_mode=padding_mode, upsample_mode=upsample_mode, out_size=list_res.pop()))
        
        # Out layer
        self.ConvsUp.append(_ConvBlock3D(out_fmaps, 'out', self.kernel_sizes[0],
                padding_mode=padding_mode))
