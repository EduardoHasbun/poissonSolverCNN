...
class UNet3D(nn.Module):
    def __init__(self, scales, kernel,input_res):
        ...
        self.ConvBottom = _ConvBlock3D(scales[4][0], kernel, pool=True, upsample_size=(int(input_res/9), int(input_res/9), int(input_res/9)))

        self.ConvsUp = nn.ModuleList([
            _ConvBlock3D(scales[1][1], kernel, upsample_size=(int(input_res/4), int(input_res/4), int(input_res/2))),
