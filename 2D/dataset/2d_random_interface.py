...
def generate_random(i):
    ...
    z_lower_inside = 2 * np.random.random((nnx_lower, nny_lower)) - 1
    z_lower_outside = 2 * np.random.random((nnx_lower, nny_lower)) - 1
    f_inside = rgi((x_lower, y_lower,), z_lower_inside, method='cubic')
    f_outside = rgi((x_lower, y_lower,), z_lower_outside, method='cubic')
    return f_inside(points).reshape((nnx, nny)), f_outside(points).reshape((nnx, nny))
